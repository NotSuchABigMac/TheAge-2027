...
adjustment (`ANTHEM_STROKE_ADJUSTMENT` in `scoring.js`) is applied via
`anthemAdjustedHandicap(hcp, sang)` *after* the daily-handicap conversion
below (i.e. against that player's Black Bull course handicap, not their
raw index) — still the last thing applied before `scrambleTeamHandicap()`
in `day2GroupHandicap()`, still per-player per the confirmed "individual"
scope, but ordered so the fixed +2/-1 house-rule strokes land exactly as
described instead of getting scaled by Black Bull's slope.
